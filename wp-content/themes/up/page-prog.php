...
ob_start(); ?>
  <div class="prog">
    <div class="prog-filter">
      <form action="">
        <div class="row cat">
          <label for="prog_filter_type"><?php echo __('Filtre por uma categoria', 'up')?>:</label>
          <select name="categoria" id="prog_filter_type" name="categoria" onchange="this.form.submit(); ">
            <option value=""><?php echo __('Todas as categorias', 'up') ?></option>
            <?php
            /*$categories = get_terms([
              'taxonomy' => 'cat_activities',
              'hide_empty' => true,
            ]);*/
            $categories = get_terms_by_post_type(['cat_activities'], ['prog']);
            foreach ($categories as $cat): ?>
              <option <?php echo $cat_filter == $cat->slug ? 'selected' : ''; ?>
        ...
        <div class="row buttons">
          ...
          <div class="days">
            <button value="" class="button button-white-outline <?php echo !$day_filter ? 'active' : '' ?> "><span><?php echo __('todos', 'up')?></span></button>
            <?php
            $prog_event_init = \DateTime::createFromFormat('Y-m-d', '2024-06-19');
            $prog_event_end = \DateTime::createFromFormat('Y-m-d', '2024-06-24');
            while ($prog_event_init <= $prog_event_end) {
              echo sprintf(
                "<button value='%s' name='dia' class='button button-white-outline %s' type='submit'><span>%s</span></button>",
                $prog_event_init->format('Y-m-d'),
                $day_filter == $prog_event_init->format('Y-m-d') ? 'active' : '',
                str_replace(['setembro', 'outubro', 'janeiro', 'junho'], ['set', 'out', 'jan', 'jun'], date_i18n('d F', $prog_event_init->getTimestamp()))
              );
    ...
    <div class="prog-items">
      ...
      if( !empty($prog_per_days) ) {
        foreach ($prog_per_days as $day => $prog_items) {
          ...
          echo '<div class="prog-day"><span>'.str_replace(['setembro', 'outubro', 'janeiro'], ['set', 'out', 'jan'], date_i18n('d F', $prog_items['start_date_time']->getTimestamp())).'</span></div>';
          ...
          foreach ($prog_items['items'] as $prog) :
            ...
            <div class="prog-item">
              <a href="<?php echo $prog_real_link ?>">
                <figure>
                  <?php if (!empty($prog_img['ID'])) { echo wp_get_attachment_image($prog_img['ID'], 'prog'); } ?>
                </figure>
                <div class="wrapper">
                  <div class="label">
                  ...
                  <div class="wrapper-bgc">
                    ...
                    <?php if ($prog_real_link): ?>
                      <span class="button button-primary"><span>Saiba Mais</span></span>
                    <?php endif; ?>
                  </div>
                </div>
              </a>
        ...
        echo '<h2>Nada por aqui :( Por favor, tente novamente.</h2>';
      }
      ?>
    </div>
  </div>
